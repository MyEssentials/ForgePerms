...
public abstract class PermissionsCommand implements CommandListener {
	...
	protected void informGroup(Object plugin, PermissionGroup group, String message) {
		for (PermissionUser user : group.getUsers()) {
			informPlayer(plugin, user.getName(), message);
		}
	}

	protected void informPlayer(Object plugin, String playerName, String message) {
		if (!(plugin instanceof PermissionsEx) || !((PermissionsEx) plugin).getConfig().get("permissions", "informplayers_changes", false).getBoolean(false)) {
	...
	protected void printEntityInheritance(ICommandSender sender, PermissionGroup[] groups) {
	...
	protected String autoCompletePlayerName(String playerName, String argName) {
		...
		for (String player : MinecraftServer.getServer().getConfigurationManager().getAllUsernames()) {
			...
			if (player.toLowerCase().startsWith(playerName.toLowerCase()) && !players.contains(player)) {
				players.add(player);
			}
		}
	...
	protected String autoCompleteWorldName(String worldName, String argName) {
		...
		for (WorldServer world : MinecraftServer.getServer().worldServers) {
			...
			if (name.toLowerCase().startsWith(worldName.toLowerCase()) && !worlds.contains(name)) {
				worlds.add(name);
			}
		}
	...
	protected String getSafeWorldName(String worldName, String userName) {
		if (worldName == null) {
			...
			if (player != null) {
				worldName = String.valueOf(player.worldObj.provider.dimensionId);
			} else {
				worldName = "0";
			}
		}
	...
	protected String autoCompletePermission(IPermissionEntity entity, String permission, String worldName) {
		return this.autoCompletePermission(entity, permission, worldName, "permission");
	}

	protected String autoCompletePermission(IPermissionEntity entity, String permission, String worldName, String argName) {
		if (permission == null) {
			return permission;
		}
	...
	protected String mapPermissions(String worldName, IPermissionEntity entity, int level) {
		StringBuilder builder = new StringBuilder();

		int index = 1;
		for (String permission : getPermissionsTree(entity, worldName, 0)) {
			if (level > 0) {
				builder.append("   ");
			} else {
	...
	protected List<String> getPermissionsTree(IPermissionEntity entity, String world, int level) {
