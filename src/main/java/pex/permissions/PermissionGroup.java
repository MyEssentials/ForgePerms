 ...
 * @author t3hk0d3
 */
public abstract class PermissionGroup extends PermissionEntity implements
    ...
    public void initialize() {
        ...
        if (this.isDebug()) {
            Logger.getLogger("Minecraft").info(
                    "[PermissionsEx] Group " + this.getName() + " initialized");
        }
    }
    ...
    public int getWeight() {
        if (dirtyWeight) {
            weight = this.getOptionInteger("weight", null, 0);
            dirtyWeight = false;
        }
    ...
    public PermissionGroup[] getParentGroups(String worldName) {
        ...
        for (String parentGroup : this.getParentGroupsNamesImpl(worldName)) {
            ...
            if (!parentGroups.contains(group)
                    && !group.isChildOf(this, worldName, true)) { // To prevent
                                                                  // cyclic
                                                                  // inheritance
        ...
        if (worldName != null) {
            ...
            for (String parentWorld : manager.getWorldInheritance(worldName)) {
                parentGroups
                        .addAll(Arrays.asList(getParentGroups(parentWorld)));
            }
    ...
    protected PermissionGroup[] getWorldGroups(String worldName) {
        ...
        for (String groupName : this.getParentGroupsNamesImpl(worldName)) {
            if (groupName == null || groupName.isEmpty()
                    || groupName.equalsIgnoreCase(this.getName())) {
                continue;
            }
    ...
    public boolean isChildOf(PermissionGroup group, String worldName,
        ...
        for (PermissionGroup parentGroup : this.getParentGroups(worldName)) {
            ...
            if (checkInheritance
                    && parentGroup
                            .isChildOf(group, worldName, checkInheritance)) {
                return true;
            }
        }
    ...
    public boolean isChildOf(String groupName, String worldName,
            boolean checkInheritance) {
        return isChildOf(manager.getGroup(groupName), worldName,
                checkInheritance);
    }
    ...
    public PermissionGroup[] getChildGroups(String worldName) {
        return manager.getGroups(this.getName(), worldName, false);
    }

    public PermissionGroup[] getChildGroups() {
        return manager.getGroups(this.getName(), false);
    }
    ...
    public PermissionGroup[] getDescendantGroups(String worldName) {
        return manager.getGroups(this.getName(), worldName, true);
    }

    public PermissionGroup[] getDescendantGroups() {
        return manager.getGroups(this.getName(), true);
    }
    ...
    public PermissionUser[] getUsers(String worldName) {
        return manager.getUsers(this.getName(), worldName, false);
    }

    public PermissionUser[] getUsers() {
        return manager.getUsers(this.getName());
    }

    public boolean isDefault(String worldName) {
        return this.equals(manager.getDefaultGroup(worldName));
    }
    ...
    protected void getInheritedPermissions(String worldName,
            ...
            boolean worldInheritance, boolean firstStep) {
        if (firstStep) {
            permissions.addAll(Arrays.asList(this
                    .getTimedPermissions(worldName)));
            permissions
                    .addAll(Arrays.asList(this.getOwnPermissions(worldName)));
        } else { // filter permissions for ancestors groups
            this.copyFilterPermissions(NON_INHERITABLE_PREFIX, permissions,
                    this.getTimedPermissions(worldName));
            this.copyFilterPermissions(NON_INHERITABLE_PREFIX, permissions,
                    this.getOwnPermissions(worldName));
        }
    ...
    protected void copyFilterPermissions(String filterPrefix, List<String> to,
            String[] from) {
