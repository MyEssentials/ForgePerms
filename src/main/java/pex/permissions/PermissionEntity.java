...
public abstract class PermissionEntity implements IPermissionEntity {
	...
	public void initialize() {
		debugMode = getOptionBoolean("debug", null, debugMode);
	}
	...
	public boolean has(String permission, String dimension) {
		...
		if (isDebug()) {
			Logger.getLogger("Minecraft").info("User " + getName() + " checked for \"" + permission + "\", " + (expression == null ? "no permission found" : "\"" + expression + "\" found"));
		}
	...
	public void removePermission(String permission) {
		for (String world : getAllPermissions().keySet()) {
	...
	public int getTimedPermissionLifetime(String permission, String world) {
		...
		return (int) (timedPermissionsTime.get(world + ":" + permission).longValue() - System.currentTimeMillis() / 1000L);
	}
	...
	public String getMatchingExpression(String permission, String world) {
		return this.getMatchingExpression(getPermissions(world), permission);
	}
