...
public abstract class PermissionBackend {
    ...
    protected PermissionBackend(PermissionManager manager, Configuration config) {
        ...
        createUserRecords = config.get("permissions", "createUserRecords",
                createUserRecords).getBoolean(createUserRecords);
    }
    ...
    public boolean removeGroup(String groupName) {
        ...
        manager.getGroup(groupName).remove();

        return true;
    }

    /**
     * Returns default group, a group that is assigned to a user without a group
     * set
     * 
     ...
     * @param world
     *            world name
     * @return Array of parent worlds. If there is no parent world return empty
     *         array
     */
    ...
    public PermissionGroup[] getGroups(String groupName, boolean inheritance) {
        ...
        for (WorldServer world : MinecraftServer.getServer().worldServers) {
            groups.addAll(Arrays.asList(getGroups(groupName, String
                    .valueOf(world.provider.dimensionId), inheritance)));
        }
    ...
    public PermissionGroup[] getGroups(String groupName, String worldName,
        ...
        for (PermissionGroup group : this.getGroups()) {
            if (!groups.contains(group)
                    && group.isChildOf(groupName, worldName, inheritance)) {
                groups.add(group);
            }
        }
    ...
    public PermissionUser[] getUsers() {
        ...
        for (String player : MinecraftServer.getServer()
            ...
            users.add(manager.getUser(player));
        }
    ...
    public static Class<? extends PermissionBackend> getBackendClass(
    ...
    public static void registerBackendAlias(String alias,
        ...
        if (!PermissionBackend.class.isAssignableFrom(backendClass)) {
            throw new RuntimeException(
                    "Provided class should be subclass of PermissionBackend");
        }

        registedAliases.put(alias, backendClass);

        Logger.getLogger("Minecraft").info(
                "[PermissionsEx] " + alias + " backend registered!");
    }
     ...
     *         backendClass.getName()
     */
    public static String getBackendAlias(
            Class<? extends PermissionBackend> backendClass) {
        if (registedAliases.containsValue(backendClass)) {
            for (String alias : registedAliases.keySet()) { // Is there better
                                                            // way to find key
                                                            // by value?
    ...
    public static PermissionBackend getBackend(String backendName,
        ...
        return getBackend(backendName, PermissionsEx.getPermissionManager(),
     ...
     */
    public static PermissionBackend getBackend(String backendName,
            PermissionManager manager, Configuration config) {
     ...
     *            name of backend that should be used if specified backend was
    ...
    public static PermissionBackend getBackend(String backendName,
        ...
        try {
            ...
            Logger.getLogger("Minecraft").info(
                    "[PermissionsEx] Initializing " + backendName + " backend");

            Constructor<? extends PermissionBackend> constructor = backendClass
                    .getConstructor(PermissionManager.class,
                            Configuration.class);
            return constructor.newInstance(manager, config);
        } catch (ClassNotFoundException e) {

            Logger.getLogger("Minecraft").warning(
                    "[PermissionsEx] Specified backend \"" + backendName
                            + "\" are not found.");
