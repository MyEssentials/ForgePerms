...
public final class ParserImpl implements Parser {
    ...
    private class ParseStreamStart implements Production {
        ...
        public Event produce() {
            ...
            Event event = new StreamStartEvent(token.getStartMark(), token
                    .getEndMark());
    ...
    private class ParseImplicitDocumentStart implements Production {
        ...
        public Event produce() {
            ...
            if (!scanner.checkToken(Token.ID.Directive, Token.ID.DocumentStart,
                ...
                Event event = new DocumentStartEvent(startMark, endMark, false,
                        null, null);
                // Prepare the next state.
    ...
    private class ParseDocumentStart implements Production {
        ...
        public Event produce() {
            ...
            if (!scanner.checkToken(Token.ID.StreamEnd)) {
                ...
                event = new DocumentStartEvent(startMark, endMark, true, tuple
                        .getVersion(), tuple.getTags());
                states.push(new ParseDocumentEnd());
                state = new ParseDocumentContent();
            } else {
                // Parse the end of the stream.
                StreamEndToken token = (StreamEndToken) scanner.getToken();
                event = new StreamEndEvent(token.getStartMark(), token
                        .getEndMark());
                if (!states.isEmpty()) {
                    throw new YAMLException(
                            "Unexpected end of stream. States left: " + states);
                }
                if (!marks.isEmpty()) {
                    throw new YAMLException(
                            "Unexpected end of stream. Marks left: " + marks);
                }
                state = null;
            }
            return event;
    ...
    private VersionTagsTuple processDirectives() {
        ...
        while (scanner.checkToken(Token.ID.Directive)) {
            ...
            if (token.getName().equals("YAML")) {
                if (yamlVersion != null) {
                    throw new ParserException(null, null,
                            "found duplicate YAML directive", token
                                    .getStartMark());
                }
                ...
                if (tagHandles.containsKey(handle)) {
                    throw new ParserException(null, null,
                            "duplicate tag handle " + handle, token
                                    .getStartMark());
                }
    ...
    private Event parseNode(boolean block, boolean indentlessSequence) {
        ...
        if (scanner.checkToken(Token.ID.Alias)) {
            ...
            event = new AliasEvent(token.getValue(), token.getStartMark(),
                    token.getEndMark());
            state = states.pop();
        } else {
            String anchor = null;
                ...
                if (handle != null) {
                    if (!directives.getTags().containsKey(handle)) {
                        throw new ParserException("while parsing a node",
                                startMark, "found undefined tag handle "
                                        + handle, tagMark);
                    }
            ...
            if (indentlessSequence && scanner.checkToken(Token.ID.BlockEntry)) {
                ...
                event = new SequenceStartEvent(anchor, tag, implicit,
                        startMark, endMark, Boolean.FALSE);
                ...
                if (scanner.checkToken(Token.ID.Scalar)) {
                    ...
                    event = new ScalarEvent(anchor, tag, implicitValues, token
                            .getValue(), startMark, endMark, token.getStyle());
                    state = states.pop();
                } else if (scanner.checkToken(Token.ID.FlowSequenceStart)) {
                    endMark = scanner.peekToken().getEndMark();
                    event = new SequenceStartEvent(anchor, tag, implicit,
                            startMark, endMark, Boolean.TRUE);
                    ...
                    state = new ParseFlowMappingFirstKey();
                        ...
                        && scanner.checkToken(Token.ID.BlockSequenceStart)) {
                    endMark = scanner.peekToken().getStartMark();
                    event = new SequenceStartEvent(anchor, tag, implicit,
                            startMark, endMark, Boolean.FALSE);
                    state = new ParseBlockSequenceFirstEntry();
                } else if (block
                        && scanner.checkToken(Token.ID.BlockMappingStart)) {
                    endMark = scanner.peekToken().getStartMark();
                    event = new MappingStartEvent(anchor, tag, implicit,
                            startMark, endMark, Boolean.FALSE);
                    ...
                    // specified.
                    event = new ScalarEvent(anchor, tag, new ImplicitTuple(
                            implicit, false), "", startMark, endMark, (char) 0);
                    state = states.pop();
                } else {
                    String node;
                    if (block) {
    ...
    private class ParseBlockSequenceEntry implements Production {
        ...
        public Event produce() {
            ...
            if (!scanner.checkToken(Token.ID.BlockEnd)) {
                Token token = scanner.peekToken();
                throw new ParserException("while parsing a block collection",
                        marks.pop(), "expected <block end>, but found "
                                + token.getTokenId(), token.getStartMark());
            }
            Token token = scanner.getToken();
            Event event = new SequenceEndEvent(token.getStartMark(), token
                    .getEndMark());
            state = states.pop();
            marks.pop();
            return event;
    ...
    private class ParseIndentlessSequenceEntry implements Production {
        ...
        public Event produce() {
            if (scanner.checkToken(Token.ID.BlockEntry)) {
                Token token = scanner.getToken();
                if (!scanner.checkToken(Token.ID.BlockEntry, Token.ID.Key,
                        Token.ID.Value, Token.ID.BlockEnd)) {
    ...
    private class ParseBlockMappingKey implements Production {
        ...
        public Event produce() {
            if (scanner.checkToken(Token.ID.Key)) {
                Token token = scanner.getToken();
                if (!scanner.checkToken(Token.ID.Key, Token.ID.Value,
                        Token.ID.BlockEnd)) {
            ...
            if (!scanner.checkToken(Token.ID.BlockEnd)) {
                Token token = scanner.peekToken();
                throw new ParserException("while parsing a block mapping",
                        marks.pop(), "expected <block end>, but found "
                                + token.getTokenId(), token.getStartMark());
            }
            Token token = scanner.getToken();
            Event event = new MappingEndEvent(token.getStartMark(), token
                    .getEndMark());
            state = states.pop();
            marks.pop();
            return event;
    ...
    private class ParseFlowSequenceEntry implements Production {
        ...
        public Event produce() {
            if (!scanner.checkToken(Token.ID.FlowSequenceEnd)) {
                ...
                if (scanner.checkToken(Token.ID.Key)) {
                    Token token = scanner.peekToken();
                    Event event = new MappingStartEvent(null, null, true, token
                            .getStartMark(), token.getEndMark(), Boolean.TRUE);
    ...
    private Event processEmptyScalar(Mark mark) {
        return new ScalarEvent(null, null, new ImplicitTuple(true, false), "",
                mark, mark, (char) 0);
    }
}
