...
public class PerlinNoiseGenerator extends NoiseGenerator {
    ...
    protected PerlinNoiseGenerator() {
        int p[] = { 151, 160, 137, 91, 90, 15, 131, 13, 201, 95, 96, 53, 194,
                ...
                18, 169, 200, 196, 135, 130, 116, 188, 159, 86, 164, 100, 109,
    ...
    public double noise(double x, double y, double z) {
        ...
        return lerp(fZ, lerp(fY, lerp(fX, grad(perm[AA], x, y, z), grad(
                perm[BA], x - 1, y, z)), lerp(fX, grad(perm[AB], x, y - 1, z),
                grad(perm[BB], x - 1, y - 1, z))),
                lerp(fY, lerp(fX, grad(perm[AA + 1], x, y, z - 1), grad(
                        perm[BA + 1], x - 1, y, z - 1)), lerp(fX, grad(
                        perm[AB + 1], x, y - 1, z - 1), grad(perm[BB + 1],
                        x - 1, y - 1, z - 1))));
    }
     ...
     */
    public static double getNoise(double x, int octaves, double frequency,
            double amplitude) {
     ...
     * @return Resulting noise
     */
    public static double getNoise(double x, double y, int octaves,
     ...
     * @return Resulting noise
     */
    public static double getNoise(double x, double y, double z, int octaves,
            double frequency, double amplitude) {
