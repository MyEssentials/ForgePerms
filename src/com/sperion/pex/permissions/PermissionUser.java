...
public abstract class PermissionUser extends PermissionEntity {
    ...
    public void initialize() {
        ...
        if (manager.getBackend().isCreateUserRecords() && this.isVirtual()) {
        ...
        if (this.isDebug()) {
            Logger.getLogger("Minecraft").info(
                    "[PermissionsEx] User " + this.getName() + " initialized");
        }
    }

    /**
     * Return non-inherited user prefix. This means if a user don't have has own
     * prefix then empty string or null would be returned
     * 
    ...
    /**
     * Return non-inherited suffix prefix. This means if a user don't has own
    ...
    @Override
    public String getOption(String optionName, String worldName,
            String defaultValue) {
        String cacheIndex = worldName + "|" + optionName;

        if (cachedOptions.containsKey(cacheIndex)) {
            return cachedOptions.get(cacheIndex);
        }

        String value = this.getOwnOption(optionName, worldName, null);
        if (value != null) {
            cachedOptions.put(cacheIndex, value);
            return value;
        }
        ...
        for (PermissionGroup group : this.getGroups(worldName)) {
            ...
            if (value != null) {
                cachedOptions.put(cacheIndex, value); // put into cache
                                                      // inherited value
                return value;
            }
        }
    ...
    public int getOwnOptionInteger(String optionName, String world,
            int defaultValue) {
        String option = this.getOwnOption(optionName, world, Integer
    ...
    public boolean getOwnOptionBoolean(String optionName, String world,
            boolean defaultValue) {
        String option = this.getOwnOption(optionName, world, Boolean
                .toString(defaultValue));
    ...
    public double getOwnOptionDouble(String optionName, String world,
            double defaultValue) {
        String option = this.getOwnOption(optionName, world, Double
                .toString(defaultValue));
    ...
    public PermissionGroup[] getGroups(String worldName) {
        if (!cachedGroups.containsKey(worldName)) {
            cachedGroups.put(worldName, this.getGroups(worldName, manager
                    .getDefaultGroup(worldName)));
        }

        return cachedGroups.get(worldName).toArray(new PermissionGroup[0]);
    }

    private List<PermissionGroup> getGroups(String worldName,
    ...
    public void addGroup(String groupName, String worldName) {
        ...
        List<String> groups = new ArrayList<String>(Arrays.asList(this
                .getGroupsNamesImpl(worldName)));
    ...
    public void addGroup(String groupName, String worldName, long lifetime) {
        ...
        if (lifetime > 0) {
            this.setOption("group-" + groupName + "-until", Long
                    .toString(System.currentTimeMillis() / 1000 + lifetime),
                    worldName);
        }
    }
    ...
    public void removeGroup(String groupName) {
        this.removeGroup(manager.getGroup(groupName));
    }
     ...
     *            if true then descendant groups of the given group would be
     *            checked too
    ...
    public boolean inGroup(PermissionGroup group, String worldName,
        ...
        for (PermissionGroup parentGroup : this.getGroups(worldName)) {
            ...
            if (checkInheritance
                    && parentGroup.isChildOf(group, worldName, true)) {
                return true;
            }
        }
     ...
     * 
     * @param groupName
     *            group's name to check
     * @param worldName
     * @param checkInheritance
     *            if true than descendant groups of specified group would be
     *            checked too
     * @return true on success, false otherwise
     */
    public boolean inGroup(String groupName, String worldName,
            boolean checkInheritance) {
        return this.inGroup(manager.getGroup(groupName), worldName,
                checkInheritance);
    }

    public boolean inGroup(String groupName, boolean checkInheritance) {
        return this.inGroup(manager.getGroup(groupName), checkInheritance);
    }
     ...
     */
    public boolean inGroup(String groupName, String worldName) {
        return this.inGroup(manager.getGroup(groupName), worldName, true);
    }
     ...
     * 
     * @param promoter
     *            null if action is performed from console or by a plugin
     * @param ladderName
     *            Ladder name
    ...
    public PermissionGroup promote(PermissionUser promoter, String ladderName)
        ...
        for (Map.Entry<Integer, PermissionGroup> entry : manager.getRankLadder(
                ladderName).entrySet()) {
            int groupRank = entry.getValue().getRank();
            if (groupRank >= rank) { // group have equal or lower than current
                                     // rank
                continue;
            }

            if (groupRank <= promoterRank) { // group have higher rank than
                                             // promoter
                continue;
            }

            if (targetGroup != null && groupRank <= targetGroup.getRank()) { // group
                                                                             // have
                                                                             // higher
                                                                             // rank
                                                                             // than
        ...
        if (targetGroup == null) {
            throw new RankingException("User are not promoteable", this,
                    promoter);
        }
     ...
     * 
     * @param promoter
     *            Specify null if action performed from console or by plugin
     * @param ladderName
     * @throws RankingException
     */
    public PermissionGroup demote(PermissionUser demoter, String ladderName)
            throws RankingException {
        if (ladderName == null || ladderName.isEmpty()) {
            ladderName = "default";
        }
        ...
        for (Map.Entry<Integer, PermissionGroup> entry : manager.getRankLadder(
                ladderName).entrySet()) {
            int groupRank = entry.getValue().getRank();
            if (groupRank <= rank) { // group have equal or higher than current
                                     // rank
                continue;
            }
    ...
    public String[] getPermissions(String worldName) {
        if (!cachedPermissions.containsKey(worldName)) {
            List<String> permissions = new LinkedList<String>();
            this.getInheritedPermissions(worldName, permissions, true, false);

            cachedPermissions
                    .put(worldName, permissions.toArray(new String[0]));
        }

        return cachedPermissions.get(worldName);
    }

    @Override
    public void addPermission(String permission, String worldName) {
        List<String> permissions = new LinkedList<String>(Arrays.asList(this
                .getOwnPermissions(worldName)));
    ...
    protected void getInheritedPermissions(String worldName,
            List<String> permissions, boolean groupInheritance,
        ...
        if (worldName != null) {
            ...
            // Common permissions
            if (!worldInheritance) { // skip common world permissions if we are
                                     // inside world-inheritance tree
        ...
        if (groupInheritance) {
            for (PermissionGroup parentGroup : this.getGroups(worldName)) {
                parentGroup.getInheritedPermissions(worldName, permissions,
                        true, false, true);
            }
        }
    }
    ...
    protected int getPromoterRankAndCheck(PermissionUser promoter,
            String ladderName) throws RankingException {
        if (!this.isRanked(ladderName)) { // not ranked
            throw new RankingException("User are not in this ladder", this,
                    promoter);
        }
        ...
        if (promoter != null && promoter.isRanked(ladderName)) {
            ...
            if (promoterRank >= rank) {
                throw new RankingException(
                        "Promoter don't have high enough rank to change "
                                + this.getName() + "'s rank", this, promoter);
            }
        }
    ...
    protected void swapGroups(PermissionGroup src, PermissionGroup dst) {
        List<PermissionGroup> groups = new ArrayList<PermissionGroup>(Arrays
                .asList(this.getGroups()));
    ...
    public String getPrefix(String worldName) {
        ...
        if (!cachedPrefix.containsKey(worldName)) {
            String localPrefix = this.getOwnPrefix(worldName);

            if (worldName != null
                    && (localPrefix == null || localPrefix.isEmpty())) {
                // World-inheritance
                for (String parentWorld : manager
                        .getWorldInheritance(worldName)) {
            ...
            localPrefix = localPrefix.replaceAll("\\$", "§");
            cachedPrefix.put(worldName, localPrefix);
        }

        return cachedPrefix.get(worldName);
    }

    @Override
    public boolean has(String permission) {
        EntityPlayer player = MinecraftServer.getServer()
                .getConfigurationManager().getPlayerForUsername(this.getName());
        if (player != null) {
            return this.has(permission, String.valueOf(player.dimension));
        }
    ...
    public String getSuffix(String worldName) {
        // @TODO This method should be refactored
        if (!cachedSuffix.containsKey(worldName)) {
            String localSuffix = this.getOwnSuffix(worldName);

            if (worldName != null
                    && (localSuffix == null || localSuffix.isEmpty())) {
                // World-inheritance
                for (String parentWorld : manager
                        .getWorldInheritance(worldName)) {
            ...
            localSuffix = localSuffix.replaceAll("\\$", "§");
            cachedSuffix.put(worldName, localSuffix);
        }

        return cachedSuffix.get(worldName);
    }

    @Override
    public String getMatchingExpression(String permission, String world) {
        String cacheId = world + ":" + permission;
        if (!cachedAnwsers.containsKey(cacheId)) {
            cachedAnwsers.put(cacheId, super.getMatchingExpression(permission,
                    world));
        }

        return cachedAnwsers.get(cacheId);
    }
    ...
    public boolean explainExpression(String expression) {
        if (expression == null && manager.allowOps) {
            EntityPlayer player = MinecraftServer.getServer()
                    .getConfigurationManager().getPlayerForUsername(
                            this.getName());
            if (player != null
                    && MinecraftServer.getServer().getConfigurationManager()
                            .getOps().contains(player.username.toLowerCase())) {
                return true;
            }
        }
    ...
    protected boolean checkMembership(PermissionGroup group, String worldName) {
        int groupLifetime = this.getOwnOptionInteger("group-" + group.getName()
                + "-until", worldName, 0);

        if (groupLifetime > 0
                && groupLifetime < System.currentTimeMillis() / 1000) { // check
                                                                        // for
                                                                        // expiration
            this.setOption("group-" + group.getName() + "-until", null,
                    worldName); // remove option
