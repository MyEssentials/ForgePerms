 ...
 */
package com.sperion.pex.permissions.backends;
...
import com.sperion.pex.permissions.backends.sql.SQLConnection;
import com.sperion.pex.permissions.backends.sql.SQLEntity;
...
public class SQLBackend extends PermissionBackend {
    ...
    public SQLBackend(PermissionManager manager,
            net.minecraftforge.common.Configuration config) {
        super(manager, config);
    }

    @Override
    public void initialize() {
        String dbDriver = config.get("permissions", "backends_sql_driver",
                "mysql").getString();
        String dbUri = config.get("permissions", "backends_sql_uri",
                "mysql://localhost/exampledb").getString();
        String dbUser = config.get("permissions", "backends_sql_user",
                "databaseuser").getString();
        String dbPassword = config.get("permissions", "backends_sql_password",
                "databasepassword").getString();

        sql = new SQLConnection(dbUri, dbUser, dbPassword, dbDriver);

        Logger.getLogger("Minecraft").info(
                "[PermissionsEx-SQL] Successfuly connected to database");
    ...
    public PermissionGroup getDefaultGroup(String worldName) {
        try {
            ...
            if (worldName == null) {
                result = sql
                        .select("SELECT `name` FROM `permissions_entity` WHERE `type` = ? AND `default` = 1 LIMIT 1",
                                SQLEntity.Type.GROUP.ordinal());
                ...
                result = sql
                        .select("SELECT `name` FROM `permissions` WHERE `permission` = 'default' AND `value` = 'true' AND `type` = ? AND `world` = ?",
    ...
    public void setDefaultGroup(PermissionGroup group, String worldName) {
        try {
            if (worldName == null) {
                // Reset default flag
                sql.executeUpdate(
                        "UPDATE `permissions_entity` SET `default` = 0 WHERE `type` = ? AND `default` = 1 LIMIT 1",
                        SQLEntity.Type.GROUP.ordinal());
                // Set default flag
                sql.executeUpdate(
                        "UPDATE `permissions_entity` SET `default` = 1 WHERE `type` = ? AND `name` = ? LIMIT 1",
                        SQLEntity.Type.GROUP.ordinal(), group.getName());
            } else {
                sql.executeUpdate(
                        "DELETE FROM `permissions` WHERE `permission` = 'default' AND `world` = ? AND `type` = ?",
                        worldName, SQLEntity.Type.GROUP.ordinal());
                sql.executeUpdate(
                        "INSERT INTO `permissions` (`name`, `permission`, `type`, `world`, `value`) VALUES (?, 'default', ?, ?, 'true')",
                        group.getName(), SQLEntity.Type.GROUP.ordinal(),
                        worldName);
            }
        } catch (SQLException e) {
    ...
    public PermissionGroup[] getGroups() {
        String[] groupNames = SQLEntity.getEntitiesNames(sql,
                SQLEntity.Type.GROUP, false);
    ...
    public PermissionUser[] getRegisteredUsers() {
        String[] userNames = SQLEntity.getEntitiesNames(sql,
                SQLEntity.Type.USER, false);
    ...
    protected final void deployTables(String driver) {
        ...
        try {
            InputStream databaseDumpStream = getClass().getResourceAsStream(
                    "/sql/" + driver + ".sql");

            if (databaseDumpStream == null) {
                throw new Exception(
                        "Can't find appropriate database dump for used database ("
                                + driver + "). Is it bundled?");
            }

            String deploySQL = StringUtils.readStream(databaseDumpStream);

            Logger.getLogger("Minecraft").info(
                    "Deploying default database scheme");
            ...
            Logger.getLogger("Minecraft").info(
                    "Database scheme deploying complete.");

        } catch (Exception e) {
            Logger.getLogger("Minecraft")
                    .severe("SQL Error: " + e.getMessage());
            Logger.getLogger("Minecraft")
                    .severe("Deploying of default scheme failed. Please initialize database manually using "
                            + driver + ".sql");
        }
    }
    ...
    public void dumpData(OutputStreamWriter writer) throws IOException {
        ...
        for (PermissionUser user : manager.getUsers()) {
            ...
            for (String group : user.getGroupsNames()) {
                writer.append("INSERT INTO `permissions_inheritance` ( `child`, `parent`, `type` ) VALUES ( '"
                        + user.getName() + "', '" + group + "',  1);\n");
            }

            // Permissions
            for (Map.Entry<String, String[]> entry : user.getAllPermissions()
                    .entrySet()) {
            ...
            // Options
            for (Map.Entry<String, Map<String, String>> entry : user
                    .getAllOptions().entrySet()) {
                for (Map.Entry<String, String> option : entry.getValue()
                        .entrySet()) {
        ...
        for (PermissionGroup group : manager.getGroups()) {
            ...
            writer.append("INSERT INTO `permissions_entity` ( `name`, `type`, `prefix`, `suffix`, `default` ) VALUES ( '"
                    ...
                    + group.getOwnSuffix()
                    + "', "
                    + (group.equals(defaultGroup) ? "1" : "0") + " );\n");

            // Inheritance
            for (String parent : group.getParentGroupsNames()) {
                writer.append("INSERT INTO `permissions_inheritance` ( `child`, `parent`, `type` ) VALUES ( '"
                        + group.getName() + "', '" + parent + "',  0);\n");
            }

            // Permissions
            for (Map.Entry<String, String[]> entry : group.getAllPermissions()
                    .entrySet()) {
                for (String permission : entry.getValue()) {
                    String world = entry.getKey();
            ...
            // Options
            for (Map.Entry<String, Map<String, String>> entry : group
                    .getAllOptions().entrySet()) {
                for (Map.Entry<String, String> option : entry.getValue()
                        .entrySet()) {
                    String value = option.getValue().replace("'", "\\'");
        ...
        if (!worldInheritanceCache.containsKey(world)) {
            try {
                ResultSet result = sql
                        .select("SELECT `parent` FROM `permissions_inheritance` WHERE `child` = ? AND `type` = 2;",
                                world);
                ...
                worldInheritanceCache.put(world, worldParents
    ...
    public void setWorldInheritance(String worldName, String[] parentWorlds) {
        ...
        try {
            sql.executeUpdate(
                    "DELETE FROM `permissions_inheritance` WHERE `child` = ? AND `type` = 2",
                    worldName);
            ...
            sql.insert("permissions_inheritance", new String[] { "child",
                    "parent", "type" }, records);
