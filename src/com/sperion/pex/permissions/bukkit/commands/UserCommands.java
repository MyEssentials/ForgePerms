...
public class UserCommands extends PermissionsCommand {
	...
	public void usersList(Object plugin, ICommandSender sender, Map<String, String> args) {
		...
		sender.sendChatToPlayer(ChatColor.WHITE + "Currently registered users: ");
		for (PermissionUser user : users) {
			sender.sendChatToPlayer(" " + user.getName() + " " + ChatColor.DARK_GREEN + "[" + StringUtils.implode(user.getGroupsNames(), ", ") + "]");
		}
	}
	...
	public void userListSuperPermissions(Object plugin, ICommandSender sender, Map<String, String> args) {
		...
		if (player == null) {
			sender.sendChatToPlayer(ChatColor.RED + "Player not found (offline?)");
			return;
		}
...
/*
		for (PermissionAttachmentInfo info : player.getEffectivePermissions()) {
			...
			sender.sendChatToPlayer(" '" + ChatColor.GREEN + info.getPermission() + ChatColor.WHITE + "' = " + ChatColor.BLUE + info.getValue() + ChatColor.WHITE + " by " + ChatColor.DARK_GREEN + pluginName);
		}*/
	}
	...
	public void userPrefix(Object plugin, ICommandSender sender, Map<String, String> args) {
		...
		sender.sendChatToPlayer(user.getName() + "'s prefix = \"" + user.getPrefix() + "\"");
	}
	...
	public void userSuffix(Object plugin, ICommandSender sender, Map<String, String> args) {
		...
		sender.sendChatToPlayer(user.getName() + "'s suffix = \"" + user.getSuffix() + "\"");
	}
	...
	public void userToggleDebug(Object plugin, ICommandSender sender, Map<String, String> args) {
		...
		sender.sendChatToPlayer("Debug mode for user " + userName + " " + (user.isDebug() ? "enabled" : "disabled") + "!");
	}
	...
	public void userCheckPermission(Object plugin, ICommandSender sender, Map<String, String> args) {
		...
		if (permission == null) {
			sender.sendChatToPlayer("Player \"" + userName + "\" don't such have no permission");
		} else {
			sender.sendChatToPlayer("Player \"" + userName + "\" have \"" + permission + "\" = " + user.explainExpression(permission));
		}
	}
	...
	public void userGetOption(Object plugin, ICommandSender sender, Map<String, String> args) {
		...
		sender.sendChatToPlayer("Player " + userName + " @ " + worldName + " option \"" + args.get("option") + "\" = \"" + value + "\"");
	}
	...
	public void userDelete(Object plugin, ICommandSender sender, Map<String, String> args) {
		...
		sender.sendChatToPlayer(ChatColor.WHITE + "User \"" + user.getName() + "\" removed!");
	}
	...
	public void userAddPermission(Object plugin, ICommandSender sender, Map<String, String> args) {
		...
		sender.sendChatToPlayer(ChatColor.WHITE + "Permission \"" + args.get("permission") + "\" added!");
	...
	public void userRemovePermission(Object plugin, ICommandSender sender, Map<String, String> args) {
		...
		sender.sendChatToPlayer(ChatColor.WHITE + "Permission \"" + permission + "\" removed!");
		this.informPlayer(plugin, userName, "Your permissions have been changed!");
	}
	...
	public void userAddTimedPermission(Object plugin, ICommandSender sender, Map<String, String> args) {
		...
		sender.sendChatToPlayer(ChatColor.WHITE + "Timed permission \"" + permission + "\" added!");
		this.informPlayer(plugin, userName, "Your permissions have been changed!");
	...
	public void userRemoveTimedPermission(Object plugin, ICommandSender sender, Map<String, String> args) {
		...
		sender.sendChatToPlayer(ChatColor.WHITE + "Timed permission \"" + permission + "\" removed!");
		this.informPlayer(plugin, userName, "Your permissions have been changed!");
	}
	...
	public void userListGroup(Object plugin, ICommandSender sender, Map<String, String> args) {
		...
		sender.sendChatToPlayer("User " + args.get("user") + " @" + worldName + " currently in:");
	...
	public void userAddGroup(Object plugin, ICommandSender sender, Map<String, String> args) {
		...
		if (args.containsKey("lifetime")) {
			try {
				...
				sender.sendChatToPlayer(ChatColor.RED + "Group lifetime should be number!");
				return;
			}
		...
		sender.sendChatToPlayer(ChatColor.WHITE + "User added to group \"" + groupName + "\"!");
		...
		if (groupName.contains(",")) {
			...
			for (int i = 0; i < groupsNames.length; i++) {
				if (sender instanceof EntityPlayer && !manager.has((EntityPlayer) sender, "permissions.manage.membership." + groupsNames[i].toLowerCase())) {
					sender.sendChatToPlayer(ChatColor.RED + "Don't have enough permission for group " + groupsNames[i]);
					return;
				}
			...
			if (groupName != null) {
				...
				if (sender instanceof EntityPlayer && !manager.has((EntityPlayer) sender, "permissions.manage.membership." + groupName.toLowerCase())) {
					sender.sendChatToPlayer(ChatColor.RED + "Don't have enough permission for group " + groupName);
					return;
				}
	...
	public void userRemoveGroup(Object plugin, ICommandSender sender, Map<String, String> args) {
		...
		sender.sendChatToPlayer(ChatColor.WHITE + "User removed from group " + groupName + "!");
	...
	public void usersCleanup(Object plugin, ICommandSender sender, Map<String, String> args) {
		...
		if (args.containsKey("threshold")) {
			try {
				...
				sender.sendChatToPlayer(ChatColor.RED + "Threshold should be number (in days)");
				return;
			}
		}
