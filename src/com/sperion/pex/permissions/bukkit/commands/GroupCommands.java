...
import com.sperion.pex.utils.DateUtils;
...
public class GroupCommands extends PermissionsCommand {
    ...
    public void groupsList(Object plugin, ICommandSender sender,
            Map<String, String> args) {
        PermissionGroup[] groups = PermissionsEx.getPermissionManager()
                .getGroups();
        String worldName = this.autoCompleteWorldName(args.get("world"));

        sender.sendChatToPlayer(ChatColor.WHITE + "Registered groups: ");
        for (PermissionGroup group : groups) {
            ...
            sender.sendChatToPlayer(String.format("  %s %s %s %s[%s]", group
                    .getName(), " #" + group.getWeight(), rank,
                    ChatColor.DARK_GREEN, StringUtils.implode(group
                            .getParentGroupsNames(worldName), ", ")));
        }
    }

    @Command(name = "pex", syntax = "groups", permission = "permissions.manage.groups.list", description = "List all registered groups (alias)")
    public void groupsListAlias(Object plugin, ICommandSender sender,
            Map<String, String> args) {
    ...
    public void groupsListAnotherAlias(Object plugin, ICommandSender sender,
            Map<String, String> args) {
        this.groupsList(plugin, sender, args);
    }
    ...
    public void groupPrintSetWeight(Object plugin, ICommandSender sender,
            Map<String, String> args) {
        // String groupName = this.autoCompleteGroupName(args.get("group"));

        PermissionGroup group = PermissionsEx.getPermissionManager().getGroup(
                args.get("group"));

        if (group == null) {
            sender.sendChatToPlayer(ChatColor.RED + "Group doesn't exist");
            return;
        }
        ...
        if (args.containsKey("weight")) {
            try {
                ...
                sender.sendChatToPlayer("Error! Weight should be integer value.");
                return;
            }
        }

        sender.sendChatToPlayer("Group " + group.getName() + " have "
                + group.getWeight() + " calories.");
    }
    ...
    public void groupToggleDebug(Object plugin, ICommandSender sender,
            Map<String, String> args) {
        // String groupName = this.autoCompleteGroupName(args.get("group"));
        ...
        sender.sendChatToPlayer("Debug mode for group " + group.getName()
                + " have been " + (group.isDebug() ? "enabled" : "disabled")
                + "!");
    }
    ...
    public void groupPrefix(Object plugin, ICommandSender sender,
            Map<String, String> args) {
        // String groupName = this.autoCompleteGroupName(args.get("group"));
        String worldName = this.autoCompleteWorldName(args.get("world"));
        ...
        sender.sendChatToPlayer(group.getName() + "'s prefix = \""
                + group.getPrefix(worldName) + "\"");
    }
    ...
    public void groupSuffix(Object plugin, ICommandSender sender,
            Map<String, String> args) {
        // String groupName = this.autoCompleteGroupName(args.get("group"));
        String worldName = this.autoCompleteWorldName(args.get("world"));
        ...
        sender.sendChatToPlayer(group.getName() + "'s suffix is = \""
                + group.getSuffix(worldName) + "\"");
    }
    ...
    public void groupCreate(Object plugin, ICommandSender sender,
            Map<String, String> args) {
        PermissionGroup group = PermissionsEx.getPermissionManager().getGroup(
                args.get("group"));
        ...
        if (!group.isVirtual()) {
            sender.sendChatToPlayer(ChatColor.RED + "Group "
                    + args.get("group") + " already exists");
            return;
        }
        ...
        if (args.get("parents") != null) {
            ...
            for (String parent : parents) {
                groups.add(PermissionsEx.getPermissionManager()
                        .getGroup(parent));
            }
        ...
        sender.sendChatToPlayer(ChatColor.WHITE + "Group " + group.getName()
                + " created!");
    ...
    public void groupDelete(Object plugin, ICommandSender sender,
            Map<String, String> args) {
        String groupName = this.autoCompleteGroupName(args.get("group"));
    ...
    public void groupListParentsAlias(Object plugin, ICommandSender sender,
            Map<String, String> args) {
    ...
    public void groupListParents(Object plugin, ICommandSender sender,
            Map<String, String> args) {
        String groupName = this.autoCompleteGroupName(args.get("group"));
        String worldName = this.autoCompleteWorldName(args.get("world"));
        ...
        if (group.getParentGroups(worldName).length == 0) {
            sender.sendChatToPlayer(ChatColor.RED + "Group " + group.getName()
                    + " doesn't have parents");
            return;
        }

        sender.sendChatToPlayer("Group " + group.getName() + " parents:");

        for (PermissionGroup parent : group.getParentGroups(worldName)) {
            sender.sendChatToPlayer("  " + parent.getName());
        }
    ...
    public void groupSetParents(Object plugin, ICommandSender sender,
            Map<String, String> args) {
        String groupName = this.autoCompleteGroupName(args.get("group"));
        String worldName = this.autoCompleteWorldName(args.get("world"));
        ...
        if (args.get("parents") != null) {
            ...
            for (String parent : parents) {
                PermissionGroup parentGroup = PermissionsEx
                        .getPermissionManager().getGroup(
                                this.autoCompleteGroupName(parent));
            ...
            sender.sendChatToPlayer(ChatColor.WHITE + "Group "
                    + group.getName() + " inheritance updated!");
    ...
    public void groupAddParents(Object plugin, ICommandSender sender,
            Map<String, String> args) {
        String groupName = this.autoCompleteGroupName(args.get("group"));
        String worldName = this.autoCompleteWorldName(args.get("world"));
        ...
        if (args.get("parents") != null) {
            String[] parents = args.get("parents").split(",");
            List<PermissionGroup> groups = new LinkedList<PermissionGroup>(
                    Arrays.asList(group.getParentGroups(worldName)));
    ...
    public void groupRemoveParents(Object plugin, ICommandSender sender,
            Map<String, String> args) {
        String groupName = this.autoCompleteGroupName(args.get("group"));
        String worldName = this.autoCompleteWorldName(args.get("world"));
    ...
    public void groupListAliasPermissions(Object plugin, ICommandSender sender,
            Map<String, String> args) {
    ...
    public void groupListPermissions(Object plugin, ICommandSender sender,
            Map<String, String> args) {
        String groupName = this.autoCompleteGroupName(args.get("group"));
        String worldName = this.autoCompleteWorldName(args.get("world"));
        ...
        sender.sendChatToPlayer(groupName + " are member of:");
        ...
        for (String world : group.getAllParentGroups().keySet()) {
            ...
            printEntityInheritance(sender, group.getAllParentGroups()
                    .get(world));
        }

        sender.sendChatToPlayer("Group " + group.getName() + "'s permissions:");
        this.sendMessage(sender, this.mapPermissions(worldName, group, 0));

        sender.sendChatToPlayer("Group " + group.getName() + "'s Options: ");
        for (Map.Entry<String, String> option : group.getOptions(worldName)
                .entrySet()) {
            sender.sendChatToPlayer("  " + option.getKey() + " = \""
                    + option.getValue() + "\"");
        }
    }
    ...
    public void groupAddPermission(Object plugin, ICommandSender sender,
            Map<String, String> args) {
        String groupName = this.autoCompleteGroupName(args.get("group"));
        String worldName = this.autoCompleteWorldName(args.get("world"));
        ...
        sender.sendChatToPlayer(ChatColor.WHITE + "Permission \""
                + args.get("permission") + "\" added to " + group.getName()
                + " !");
    ...
    public void groupSetOption(Object plugin, ICommandSender sender,
            Map<String, String> args) {
        String groupName = this.autoCompleteGroupName(args.get("group"));
        String worldName = this.autoCompleteWorldName(args.get("world"));
        ...
        if (args.containsKey("value") && args.get("value").isEmpty()) {
            sender.sendChatToPlayer(ChatColor.WHITE + "Option \""
                    + args.get("option") + "\" cleared!");
        } else {
            sender.sendChatToPlayer(ChatColor.WHITE + "Option \""
                    + args.get("option") + "\" set!");
        }
    ...
    public void groupRemovePermission(Object plugin, ICommandSender sender,
            Map<String, String> args) {
        String groupName = this.autoCompleteGroupName(args.get("group"));
        String worldName = this.autoCompleteWorldName(args.get("world"));
        ...
        String permission = this.autoCompletePermission(group, args
                .get("permission"), worldName);

        group.removePermission(permission, worldName);
        group.removeTimedPermission(permission, worldName);

        sender.sendChatToPlayer(ChatColor.WHITE + "Permission \"" + permission
    ...
    public void userSwapPermission(Object plugin, ICommandSender sender,
            Map<String, String> args) {
        String groupName = this.autoCompleteGroupName(args.get("group"));
        String worldName = this.autoCompleteWorldName(args.get("world"));
        ...
        try {
            int sourceIndex = this.getPosition(this.autoCompletePermission(
                    group, args.get("permission"), worldName, "permission"),
                    permissions);
            int targetIndex = this.getPosition(this.autoCompletePermission(
                    group, args.get("targetPermission"), worldName,
                    "targetPermission"), permissions);
            ...
            sender.sendChatToPlayer("Permissions swapped!");
        } catch (Throwable e) {
    ...
    public void groupAddTimedPermission(Object plugin, ICommandSender sender,
            Map<String, String> args) {
        String groupName = this.autoCompleteGroupName(args.get("group"));
        String worldName = this.autoCompleteWorldName(args.get("world"));
        ...
        if (group == null) {
            sender.sendChatToPlayer(ChatColor.RED + "Group does not exist");
            return;
        }

        group.addTimedPermission(args.get("permission"), worldName, lifetime);

        sender.sendChatToPlayer(ChatColor.WHITE + "Timed permission added!");
        this.informGroup(plugin, group, "Your permissions have been changed!");

        logger.info("Group " + groupName + " get timed permission \""
                + args.get("permission") + "\" "
                + (lifetime > 0 ? "for " + lifetime + " seconds " : " ")
                + "from " + getSenderName(sender));
    }
    ...
    public void groupRemoveTimedPermission(Object plugin,
            ICommandSender sender, Map<String, String> args) {
        String groupName = this.autoCompleteGroupName(args.get("group"));
        String worldName = this.autoCompleteWorldName(args.get("world"));
        ...
        sender.sendChatToPlayer(ChatColor.WHITE + "Timed permission \""
                + args.get("permission") + "\" removed!");
        this.informGroup(plugin, group, "Your permissions have been changed!");
    }
    ...
    public void groupUsersList(Object plugin, ICommandSender sender,
            Map<String, String> args) {
        String groupName = this.autoCompleteGroupName(args.get("group"));
        ...
        if (users == null || users.length == 0) {
            sender.sendChatToPlayer(ChatColor.RED
                    + "Group doesn't exist or empty");
        }

        sender.sendChatToPlayer("Group " + groupName + " users:");

        for (PermissionUser user : users) {
            sender.sendChatToPlayer("   " + user.getName());
        }
    }
    ...
    public void groupUsersAdd(Object plugin, ICommandSender sender,
            Map<String, String> args) {
        String groupName = this.autoCompleteGroupName(args.get("group"));
        String worldName = this.autoCompleteWorldName(args.get("world"));
        ...
        for (String userName : users) {
            ...
            if (user == null) {
                sender.sendChatToPlayer(ChatColor.RED + "User does not exist");
                return;
            }

            user.addGroup(groupName, worldName);

            sender.sendChatToPlayer(ChatColor.WHITE + "User " + user.getName()
                    + " added to " + groupName + " !");
    ...
    public void groupUsersRemove(Object plugin, ICommandSender sender,
            Map<String, String> args) {
        String groupName = this.autoCompleteGroupName(args.get("group"));
        String worldName = this.autoCompleteWorldName(args.get("world"));
        ...
        for (String userName : users) {
            ...
            sender.sendChatToPlayer(ChatColor.WHITE + "User " + user.getName()
                    + " removed from " + args.get("group") + " !");
            this.informPlayer(plugin, userName, "You were removed from \""
                    + groupName + "\" group");
    ...
    public void groupDefaultCheck(Object plugin, ICommandSender sender,
            Map<String, String> args) {
        String worldName = this.autoCompleteWorldName(args.get("world"));

        PermissionGroup defaultGroup = PermissionsEx.getPermissionManager()
                .getDefaultGroup(worldName);
        sender.sendChatToPlayer("Default group in " + worldName + " world is "
                + defaultGroup.getName() + " group");
    }
    ...
    public void groupDefaultSet(Object plugin, ICommandSender sender,
            Map<String, String> args) {
        String groupName = this.autoCompleteGroupName(args.get("group"));
        String worldName = this.autoCompleteWorldName(args.get("world"));
        ...
        sender.sendChatToPlayer("New default group in " + worldName
                + " world is " + group.getName() + " group");
    }
}
