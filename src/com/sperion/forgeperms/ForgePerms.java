    ...
    public static void registerHandler(PermissionsBase permHandler){
        if (ForgePerms.permissionsHandler==null){
            if(permHandler.load()){
                ...
                Log.info("%s failed to load because %s", permHandler.name, permHandler.loadError);
            }
        } else{
            Log.info("PermissionHandler(%s) already loaded. Did not load %s", ForgePerms.permissionsHandler.name, permHandler.name);
        }
    }
